var utils = require("jack/utils"),
    uri = require("uri"),
    dom = require("browser/dom");

var STATUS = utils.HTTP_STATUS_MESSAGES;

var propfind_allprop = "/*[local-name()='propfind']/*[local-name()='allprop']",
    propfind_prop    = "/*[local-name()='propfind']/*[local-name()='prop']/*",
    propertyupdate   = "/*[local-name()='propertyupdate']/*[local-name()='set']/*[local-name()='prop']/* | /*[local-name()='propertyupdate']/*[local-name()='remove']/*[local-name()='prop']/*";

// var propfind_allprop = "/propfind/allprop",
//     propfind_prop    = "/propfind/prop/*",
//     propertyupdate   = "/propertyupdate/set/prop/* | /propertyupdate/remove/prop/*";
...
Controller.prototype.COPY = function() {
    ...
    var destURI = uri.parse(this.env['HTTP_DESTINATION']),
        destination = this.stripScriptName(uri.unescape(destURI.path));
...
Controller.prototype.PROPFIND = function() {
    ...
    var names;
    if (this.requestMatch(propfind_allprop).length > 0) {
        names = this.resource.propertyNames();
    } else {
        names = this.requestMatch(propfind_prop).map(function(prop) {
    ...
    this.multistatus(function(xml) {
        ...
        this.findResources().forEach(function(resource) {
            ...
            href.appendChild(doc.createTextNode("http://"+this.env['HTTP_HOST']+encodeURI(resource.fullPath)));
...
Controller.prototype.PROPPATCH = function() {
    ...
    propPatch = this.requestMatch(propertyupdate).map(function(prop) {
...
Controller.prototype.requestDocument = function() {
    try {
        if (!this.requestDoc) {
            ...
            // HACK? For Litmus. Matches elements that have named null namespaces, i.e. xmlns:bar=""
...
Controller.prototype.requestMatch = function(pattern) {
    var doc = this.requestDocument(),
        result = dom.evaluate(pattern, doc, nsMapper, dom.XPathResult.ORDERED_NODE_ITERATOR_TYPE);
    ...
    for (var status in stats) {
        ...
        props.forEach(function(p) {
            ...
            //if (value) {
                ...
                if (value instanceof dom.Node || value instanceof dom.Element)
...
Controller.prototype.responseErrors = function(xml, errors) {
    ...
    errors.forEach(function(error) {
        ...
        response.appendChild(href = doc.createElement("D:href"));
        href.appendChild(doc.createTextNode("http://"+this.env['HTTP_HOST']+encodeURI(path)));
...
Controller.prototype.mapExceptions = function(block) {
    try {
        ...
        if ((typeof Packages !== "undefined" && e.javaException instanceof Packages.java.io.FileNotFoundException) ||
            String(e).indexOf("failed to make directory") >= 0 ||
            String(e).indexOf("No such file or directory") >= 0)
            throw STATUS["Conflict"];
...
Controller.prototype.httpStatusLine = function(code) {
    return (this.env['SERVER_PROTOCOL'] || this.env['HTTP_VERSION'] || "HTTP/1.1") +
            " " + code + " " + utils.HTTP_STATUS_CODES[code];
}
...
Controller.prototype.stripScriptName = function(path) {
    if (this.env["SCRIPT_NAME"]) {
        var scriptName = utils.unescape(this.env["SCRIPT_NAME"]),
            index = path.indexOf(scriptName);
        if (index === 0)
            return path.substring(scriptName.length);
    }
    return path;
}